...
public class EAIConnector {
	...
	public EAIConnector() {
		broker_port = PropertyUtils.getInstance().getInt("org.oh.eai.connector.port", 80);
		broker_host = PropertyUtils.getInstance().getString("org.oh.eai.connector.host", "10.129.29.183") + ":"
				+ String.valueOf(broker_port);
		broker_name = PropertyUtils.getInstance().getString("org.oh.eai.connector.name", "APBK");
		client_group = PropertyUtils.getInstance().getString("org.oh.eai.connector.group", "Application_CG");
		event_waiting = PropertyUtils.getInstance().getInt("org.oh.eai.connector.waiting", 5000);
	}

	public int getRepeatNumerOfTimes() {
		return PropertyUtils.getInstance().getInt("org.oh.eai.connector.retry", 3);
	}

	public int getRepeatSleepTimes() {
		return PropertyUtils.getInstance().getInt("org.oh.eai.connector.sleep", 5000);
	}
