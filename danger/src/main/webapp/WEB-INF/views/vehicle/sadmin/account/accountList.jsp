...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#register').click(function() {
			event.preventDefault();
			$(location).attr("href", contextPath + "/vehicle/sadmin/accountRegister.do");
		});
...
// 		paramObj['order_desc_asc'] = "asc";
	...
	function bindTable(result) {
		var system_type = { '1' : '시스템관리자', '0' : '일반관리자' };
		var list = $('#account_tbody').appendTable({
			data : result.adminAccountList,
			totalCnt : result.totalCnt,
			curPage : curPage,
			listCnt : listCnt,
			column : [ 'totalIndex', 'user_id', 'is_admin', 'name', 'email', 'reg_date', 'check_box' ],
